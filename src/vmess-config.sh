...
if [[ $v2ray_transport == [45] ]]; then
	sed -i "s//$domain/; 9s/2333/443/; 12s/$old_id/$v2ray_id/; 13s/233/$alterId/" $v2ray_client_config
	if [[ $is_path ]]; then
		sed -i "27s//$path/" $v2ray_client_config
	else
		sed -i "27s///" $v2ray_client_config
	fi
else
	sed -i "s//$ip/; 9s/2333/$v2ray_port/; 12s/$old_id/$v2ray_id/; 13s/233/$alterId/" $v2ray_client_config
fi
